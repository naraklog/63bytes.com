...
const SearchBar = forwardRef<HTMLInputElement, SearchBarProps>(function SearchBar(
	{ searchQuery, onSearchChange, shortcutLabel, showSearch },
	ref
) {
	if (!showSearch) {
		return (
	...
	return (
		<div className="flex items-center relative group">
			<Search className="absolute left-3 w-4 h-4 text-black pointer-events-none transition-colors group-focus-within:text-white" />
			<input
				type="text"
				placeholder="Search..."
				ref={ref}
			...
			/>
			<div className="absolute right-3 text-black/60 text-xs font-semi-mono pointer-events-none transition-colors group-focus-within:text-white/80">
				{shortcutLabel}
			</div>
		</div>
	);
