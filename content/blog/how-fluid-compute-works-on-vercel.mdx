...
export async function POST() {
	...
	const [featureFlags, experiments] = await Promise.all([getFeatureFlags(), getExperimentBranches()]);
	await analytics.flush();
	return Response.json({ featureFlags, experiments });
}
```

When paired with `remark-gfm`, authoring these callouts in Markdown stays ergonomic—you can drop checkboxes, tables, or autolinks without custom MDX components.
